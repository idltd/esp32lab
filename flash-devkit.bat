...
if "%PORT%"=="" (
    echo [ERROR] No USB serial device found. Is the DevKit connected on a data cable?
    type "%TEMP%\esp32lab_ports.txt"
    exit /b 1
)

:flash
echo [ESP32 Lab] Flashing DevKit on %PORT%...
%CLI% upload --fqbn esp32:esp32:esp32 --port %PORT% --input-dir "%~dp0firmware\build\esp32dev" "%~dp0firmware\esp32lab"
